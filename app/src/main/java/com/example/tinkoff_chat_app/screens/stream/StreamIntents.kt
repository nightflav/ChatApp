...
sealed class StreamIntents {
    ...
    data class ShowCurrentStreamsIntent(val showSubscribed: Boolean, val onError: () -> Unit): StreamIntents()
    data class ShowCurrentStreamTopicsIntent(val stream: StreamModel, val onError: () -> Unit) : StreamIntents()
}